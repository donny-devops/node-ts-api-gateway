 ...
 *   3. cors              — CORS policy
 *   4. rateLimit         — per-client rate limiting
 *   5. jwt               — verifies Bearer tokens
 *   6. ddos              — RPS flood detection and IP banning
 *   7. sanitise          — XSS / SQLi / path-traversal stripping
 *   8. auth              — route-level auth guard
 *   9. observability     — Prometheus metrics + transaction log shipping
 *  10. proxy routes      — reverse proxy to upstream services
 *  11. health routes     — /health + /ready
 */
